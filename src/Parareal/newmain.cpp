...
int main(int argc, char **argv)
{
    ...
    // Initialize parareal
    Parareal<Convection, ConvectionField> parareal(convection, qinitial, q, conf, MPI_COMM_WORLD);
    ...
    if (conf.mode() == ModeCompare)
    {
        ...
        convection.DoRK4(qinitial, qinitial, conf.dtFine(), tauSamples*conf.timeStepsFinePerTimeSlice());
        SynchronizeCUDA();
        tauF = MPI_Wtime() - tauF;
        double tauG = MPI_Wtime();
        convection.DoEuler(qinitial, qinitial, conf.dtCoarse(), tauSamples*conf.timeStepsCoarsePerTimeSlice());
        SynchronizeCUDA();
        tauG = MPI_Wtime() - tauG;
